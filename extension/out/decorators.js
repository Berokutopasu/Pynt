...
exports.DiagnosticDecorator = void 0;
// decorators.ts
...
class DiagnosticDecorator {
    ...
    applyDecorations(editor, findings) {
        ...
        for (const [lineNum, lineFindings] of findingsByLine.entries()) {
            ...
            const range = new vscode.Range(primaryFinding.line - 1, primaryFinding.column - 1, primaryFinding.endLine - 1, Math.max(primaryFinding.endColumn, primaryFinding.column));
            ...
            switch (primaryFinding.severity) {
                case types_1.SeverityLevel.ERROR:
                    errorDecorations.push(decoration);
                    break;
                case types_1.SeverityLevel.WARNING:
                    warningDecorations.push(decoration);
                    break;
                case types_1.SeverityLevel.INFO:
                default:
    ...
    createCombinedHoverMessage(findings) {
    ...
    createSingleHoverMessage(finding) {
        const md = new vscode.MarkdownString();
        md.supportHtml = true;
        md.isTrusted = true;
        md.supportThemeIcons = true;
    ...
    formatAnalysisType(type) {
        switch (type) {
            case types_1.AnalysisType.SECURITY: return 'Sicurezza';
            case types_1.AnalysisType.BEST_PRACTICES: return 'Best Practices';
            case types_1.AnalysisType.FAULT_DETECTION: return 'Rilevamento Bug';
...
exports.DiagnosticDecorator = DiagnosticDecorator;
